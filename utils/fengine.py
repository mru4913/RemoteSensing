...
class finalEngine:
    def __init__(self, model, train_loader, valid_loader, optimizer, criterion, device, num_epochs,
                scheduler=None, log_num=20, logger=None, num_class=100, name='model', deep_supervision=True):
        self.model = model 
        ...
        with torch.set_grad_enabled(phase == 'train'):
            ...
            if self.deep_supervision:
                ...
                for output in outputs:
                    self.loss += self.criterion(output, labels.long())
            ...
            if self.deep_supervision:
                avg_pixel_acc = 0
                for output in outputs:
                    avg_pixel_acc += (output.argmax(dim=1) == labels).float().mean().item()
                avg_pixel_acc /= len(outputs)
            else:
        ...
        for epoch in range(1, self.num_epochs+1):
            ...
            for batch_idx, data in enumerate(self.train_loader):
                ...
                if (batch_idx+1) % train_log_interval == 0:
                    ...
                    self.logger.info('[Train] Epoch: {} [batch: {}/{} ({:05.2f}%)] Average loss: {:.4f}  Average accuracy: {:.4f} lr: {:.10f}'.format(epoch, 
