...
class mixEngine:
    def __init__(self, model, train_loader, valid_loader, optimizer, criterion1, criterion2, device, num_epochs,
    ...
    def _forward(self, data, phase='train'):
        ...
        with torch.set_grad_enabled(phase == 'train'):
            ...
            loss1 = self.criterion1(outputs, labels.long()) # calculate the mean loss
            loss2 = self.criterion2(outputs, labels.long())
            self.loss = (loss1 + loss2)/2
            
        avg_loss = self.loss.item()    
       
        return avg_loss, outputs.cpu().numpy()
