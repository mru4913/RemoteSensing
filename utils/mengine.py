...
class mixEngine:
    alternate_epoch = 2
    def __init__(self, model, train_loader, valid_loader, optimizer, criterion1, 
    criterion2, device, num_epochs,scheduler=None, log_num=20, logger=None, num_class=100, name='model'):
        self.model = model 
    ...
    def __repr__(self):
        ...
        engine_info['criterion1'] = self.criterion1.__class__.__name__
        engine_info['criterion2'] = self.criterion2.__class__.__name__
    ...
    def _forward(self, data, phase='train'):
        ...
        with torch.set_grad_enabled(phase == 'train'):
            ...
            # if self.epoch % self.alternate_epoch == 0:
            #     self.loss = self.criterion1(outputs, labels.long())
            # else:
            #     self.loss = self.criterion2(outputs, labels.long())

            loss1 = self.criterion1(outputs, labels.long())
            loss2 = self.criterion2(outputs, labels.long())
    ...
    def _eval_step(self):
        ...
        self.logger.info('[Valid] Average loss: {:.4f} Average accuracy: {:.4f}'.format(avg_loss, avg_pixel_acc))
    ...
    def train(self, metric='fwiou', save_model=True, save_best_model=False):
        ...
        for epoch in range(1, self.num_epochs+1):
            ...
            # if self.epoch % self.alternate_epoch == 0:
            #     self.logger.info(self.criterion1.__class__.__name__)
            # else:
            #     self.logger.info(self.criterion2.__class__.__name__)
            ...
            for batch_idx, data in enumerate(self.train_loader):
                ...
                if (batch_idx+1) % train_log_interval == 0:
                    ...
                    self.logger.info('[Train] Epoch: {} [batch: {}/{} ({:05.2f}%)] Average loss: {:.4f}  Average accuracy: {:.4f} lr: {}'.format(epoch, 
