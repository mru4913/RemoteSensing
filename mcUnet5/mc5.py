...
__all__ = ['get_mc_unet5_b0','get_mc_unet5_b1','get_mc_unet5_b2','get_mc_unet5_b3','get_mc_unet5_b4', 
            'get_mc_unet5_b5','get_mc_unet5_b6','get_mc_unet5_b7']
...
class EfficientUnet(nn.Module):
    def __init__(self, encoder, out_channels=2, deep_supervision=False):
        super().__init__()
        ...
        self.conv0_4 = D_ResBlock(self.size[0]*4+self.size[1], self.size[0])
        ...
        if self.deep_supervision:
            self.final1 = D_ResBlock(self.size[0], out_channels, rate=1, nested=False)
            self.final2 = D_ResBlock(self.size[0], out_channels, rate=1, nested=False)
            self.final3 = D_ResBlock(self.size[0], out_channels, rate=1, nested=False)
            self.final4 = D_ResBlock(self.size[0], out_channels, rate=1, nested=False)
        else:
    ...
    def forward(self, x):
        ...
        if self.deep_supervision:
            output1 = self.final1(self.up(x0_1_atten))
            output2 = self.final2(self.up(x0_2_atten))
            output3 = self.final3(self.up(x0_3_atten))
...
def get_mc_unet5_b0(out_channels=2, pretrained=True, deep_supervision=True):
    encoder = EfficientNet.encoder('efficientnet-b0', pretrained=pretrained)
    model = EfficientUnet(encoder, out_channels=out_channels, deep_supervision=deep_supervision)
    return model


def get_mc_unet5_b1(out_channels=2, pretrained=True, deep_supervision=True):
    encoder = EfficientNet.encoder('efficientnet-b1', pretrained=pretrained)
    model = EfficientUnet(encoder, out_channels=out_channels, deep_supervision=deep_supervision)
    return model

def get_mc_unet5_b2(out_channels=2, pretrained=True, deep_supervision=True):
    encoder = EfficientNet.encoder('efficientnet-b2', pretrained=pretrained)
    model = EfficientUnet(encoder, out_channels=out_channels, deep_supervision=deep_supervision)
    return model

def get_mc_unet5_b3(out_channels=2, pretrained=True, deep_supervision=True):
    encoder = EfficientNet.encoder('efficientnet-b3', pretrained=pretrained)
    model = EfficientUnet(encoder, out_channels=out_channels, deep_supervision=deep_supervision)
    return model

def get_mc_unet5_b4(out_channels=2, pretrained=True, deep_supervision=True):
    encoder = EfficientNet.encoder('efficientnet-b4', pretrained=pretrained)
    model = EfficientUnet(encoder, out_channels=out_channels, deep_supervision=deep_supervision)
    return model

def get_mc_unet5_b5(out_channels=2, pretrained=True, deep_supervision=True):
    encoder = EfficientNet.encoder('efficientnet-b5', pretrained=pretrained)
    model = EfficientUnet(encoder, out_channels=out_channels, deep_supervision=deep_supervision)
    return model

def get_mc_unet5_b6(out_channels=2, pretrained=True, deep_supervision=True):
    encoder = EfficientNet.encoder('efficientnet-b6', pretrained=pretrained)
    model = EfficientUnet(encoder, out_channels=out_channels, deep_supervision=deep_supervision)
    return model

def get_mc_unet5_b7(out_channels=2, pretrained=True, deep_supervision=True):
