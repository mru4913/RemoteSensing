...
from loss import LovaszSoftmax
from utils.data import trainDataset, myDataset
from utils.engine import Engine
from utils.fengine import finalEngine
...
def main(args):
    os.environ["CUDA_VISIBLE_DEVICES"] = '0,1'#','.join(map(str,args.cuda_device))
    ...
    if args.model.lower() == 'unet':
        ...
        model = get_scse_eunet_b4(out_channels=args.num_class, concat_input=args.concat, pretrained=args.pretrained)
    elif args.model.lower() == 'eunet_b5':
        model = get_scse_eunet_b5(out_channels=args.num_class, concat_input=args.concat, pretrained=args.pretrained)
    elif args.model.lower() == 'basic_eunet':
        model = get_efficientunet_b5(out_channels=args.num_class, concat_input=args.concat, pretrained=args.pretrained)
    elif args.model.lower() == 'res':
        model = ResNestedUNet(num_classes=args.num_class)
    elif args.model.lower() == 'scseunet':
        model = SCSEUnet_resnet34(out_channels=args.num_class, pretrained=args.pretrained)
    elif args.model.lower() == 'scse_nestedunet':
        model = scSE_ResNestedUNet(num_classes=args.num_class, deep_supervision=args.deep, 
